...
while True:
    ...
    #-----4. 통신부에 전달, 작동변수 변경    ------#
    server_data = giveserver(server_label)

    track_object = server_data['object']
    track_number = server_data['number']
    next_angle = now_angle
    for find_object in exists[track_object] :
        if find_object.number == track_number :
            next_angle = find_object.degree

    #-----5. 모터 회전 시행  [다른 것의 영향을 전혀 주지 않도록 해야해서 마지막]  ------#
    now_angle = motor.camera_motor_control(now_angle, next_angle - now_angle)
